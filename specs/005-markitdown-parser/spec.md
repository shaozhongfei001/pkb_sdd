...
| raw_vault 只读输入 | `{raw_vault_root}/by_hash/{sha256[0:2]}/{sha256[2:4]}/{sha256}/original.bin` |
...
- 路径由 `vault_path` 或 `build_vault_dir()` + `build_vault_artifact_paths()` 解析
...
6. **单条失败 continue**：失败记入 report `errors[]` 与 item `status=FAILED`。
7. **可追溯**：所有成功/失败/跳过 item 必须含 `content_uid` 与 `sha256`。
8. CLI 入口调用 `ensure_readonly()`。
9. **`--dry-run`**：不写 `parsed/`；仍输出 report（summary 反映 would-parse / would-skip）。
